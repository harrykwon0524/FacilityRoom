...
%>
    ...
    <option value = "문의"<% if(category.equals("문의")) out.println("selected");%>>공지</option>
    </select>
    ...
    ArrayList<Qna> list = dao.getQnaArray(category, sort, search, pageNum);
    if(list != null) {
    	for(int i = 0; i < list.size(); i++) {
    		Qna qna = list.get(i);
    		if(qna.getCategory().equals("공지")) {
    			if(dao.getQnaAvailable(i) == 1) {
    				%>
   ...
   <td><%= qna.getQna_no() %></td>
    ...
    %>
     ...
     if(user_id != null) {
    	%>
      <a href = "qnaadd.jsp" class = "btn btn-primary pull-right">글쓰기</a>
