...
<body>
    ...
    if(user_id == null) {
    	...
    	if(!u.getUserinfo(user_id).getState().equals("관리")) {
        	...
        	if(facility_no == -1) {
        		...
        		if( Integer.parseInt(request.getParameter("number")) == 0 ) {
                	    	...
                	    	if(d.getF(facility_no).getAmount() < Integer.parseInt(request.getParameter("number"))) {
                	    		 PrintWriter script = response.getWriter();
                    	         script.println("<script>");
    	...
    	int result = dao.update(Integer.parseInt(request.getParameter("number")), facility_no, room_no);
